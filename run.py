...
try:
    from train import evaluate, train_one_epoch, load_data, utils, load_CIFAR10
...
def prepare_model(
    ...
    dataset, dataset_test, train_sampler, test_sampler = load_data(
        traindir, valdir, _args(model=model_name, distributed=False, cache_dataset=False, val_resize_size=256, val_crop_size=256, train_crop_size=256, interpolation='nearest', weights=None))
    # dataset, dataset_test, train_sampler, test_sampler = load_CIFAR10(_args(model=model_name, distributed=False, cache_dataset=False, val_resize_size=256, val_crop_size=256, train_crop_size=256, interpolation='nearest', weights=None))
...
def main(cmdline_args):
    ...
    with torch.no_grad():
        ...
        print('Calibration took:',time.time() - st)
